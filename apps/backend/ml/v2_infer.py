...
class V2Estimator:
    ...
    def __init__(self, project_root: Path, meta_file: str = "v2_metadata.json"):
        ...
        # Data can be in backend-local layout or repository root layout.
        self.data_root = self.project_root / "data"
        if not self.data_root.exists():
            repo_data = self.project_root.parents[1] / "data"
            if repo_data.exists():
                self.data_root = repo_data
    ...
    def _make_tabular_row(self, x: Dict[str, Any]) -> Dict[str, Any]:
        ...
        object_type = _cat(x.get("object_type"))
        condition_norm = _cat(x.get("condition_norm"))
        condition_source = _cat(x.get("condition_source"))

        lat = _num(x.get("latitude"))
        lon = _num(x.get("longitude"))
        condition_confidence = _num(x.get("condition_confidence"))
        ...
        return {
            ...
            "condition_confidence": float(condition_confidence),
            "district": district,
    ...
    def _load_comparables_df(self) -> pd.DataFrame:
        idx_path = self.data_root / "index" / "index.parquet"
        if idx_path.exists():
            try:
        ...
        for raw_dir_name in ("raw_ads_test", "raw_ads"):
            raw_dir = self.data_root / raw_dir_name
    ...
    # -------------------------
    def _sanitize_clip_text(self, text: str) -> str:
        s = unicodedata.normalize("NFKC", str(text or ""))
        # Drop isolated problematic accents and control chars that may break BPE lookup.
        s = s.replace("´", "").replace("`", "").replace("’", "'")
        s = "".join(ch for ch in s if unicodedata.category(ch)[0] != "C")
        s = " ".join(s.split()).strip()
        return s or "apartment interior"

    def _encode_texts(self, texts: List[str]) -> torch.Tensor:
        try:
            tokens = self.tokenizer(texts)
        except KeyError:
            safe_texts = [self._sanitize_clip_text(t) for t in texts]
            tokens = self.tokenizer(safe_texts)
    ...
    def _renovation_zero_shot(self, emb_np: np.ndarray, threshold: float = 0.55) -> Dict[str, Any]:
        ...
        fresh_prob = float(probs[cond_keys.index("fresh")].item())
        average_prob = float(probs[cond_keys.index("average")].item())

        # Business rule:
        # keep "needs" as-is; threshold 0.335 applies only to fresh vs average boundary.
        if condition == "needs":
            confidence = float(probs[cond_keys.index("needs")].item())
        elif fresh_prob < 0.335:
            condition = "average"
