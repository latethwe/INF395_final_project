...
def build_model_payload_from_listing(rec: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "object_type": str(rec.get("object_type") or "").strip() or "unknown",
        "condition_norm": str(rec.get("condition_norm") or "").strip() or "unknown",
        "condition_source": str(rec.get("condition_source") or "").strip() or "unknown",
